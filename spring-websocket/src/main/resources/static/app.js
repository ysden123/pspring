...
/*
    var socketNotification=new SockJS('/notification');
    stompNotificationClient = Stomp.over(socketNotification);
        stompNotificationClient.connect({}, function (frame) {
            setConnected(true);
            console.log('Connected: ' + frame);
            stompNotificationClient.subscribe('/topic/notification', function (greeting) {
                showGreeting(JSON.parse(greeting.body).content);
            });
        });
*/
